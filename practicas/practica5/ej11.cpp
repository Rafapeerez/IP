...
#include <iostream>
using namespace std;
int sum_even(int v[],int n) { //Creamos la variable
    int numero=0;
    for(int cont=0; cont<n; cont++) {
        if((v[cont]%2)==0) { //Si el cociente de la division entre 2 del elemento del vector que indica el contador es 0, quiere decir que es par
            numero= numero+v[cont]; //Entonces a la variable numero se le suma cada elemento que cumpla esta condicion
        }
    }
return numero;
}
