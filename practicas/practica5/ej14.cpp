...
void ordena_vector(int v[],int n) {
    for(int i=0; i<n; i++) {   // El programa ordena de forma ascendente los elementos del vector,mediante un reconocimiento de todos los elementos con las variables i y j
        for(int j=i+1; j<n; j++) {
            if(v[i]>v[j]) {
                int aux=v[i];
                v[i]=v[j];
